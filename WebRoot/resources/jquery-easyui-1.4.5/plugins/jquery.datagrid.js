...
(function($) {
	...
	function wrapGrid(jq, rownumbers) {
		console.log("开始编织数据表格，序号的是否显示设置为："+rownumbers);
		function getColumns(thead) {
			console.log("getColumns(thead) 方法的thead="+thead);
			var columns = [];
	...
	function request(jq, param) {//yyf param是请求的参数
		var opts = $.data(jq, "datagrid").options;
		if (param) {
		...
		function doRequest() {
			console.log("通过ajax获取datagrid的数据列表");
			$.ajax( {
				type : opts.method,
				url : opts.url,
				data : param,
	...
	$.fn.datagrid = function(options, param) {
		...
		if (typeof options == "string") {
			...
			console.log(" options is not a string type! it is a "+typeof options+" type");
		}
		options = options || {};
		return this.each(
			function() {
			console.log(this);//打印标签元素e1，可能会有多个元素，所以这里用each，这里的this指当前each循环体中的这个标签元素
			var state = $.data(this, "datagrid");
			console.log(" 当前元素e1中是否具有datagrid=="+state);
			var opts;
			if (state) {
				opts = $.extend(state.options, options);//如果e1元素中确实存在datagrid数据，那么合并state.options和options，并如果options覆盖前者的同名属性
				state.options = opts;
			} else {
	...
	$.fn.datagrid.methods = {
		options : function(jq) {
			console.log(" into $.fn.datagrid.methods的options方法...");
	...
	$.fn.datagrid.parseOptions = function(jq) {
		//开始解析当前标签元素，即将当前标签解析为datagrid元素的样式和风格
		console.log(" 解析 id='"+jq.id+"'的 table标签元素");
		var t = $(jq);
