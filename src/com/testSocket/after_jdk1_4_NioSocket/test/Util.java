...
class Util {
	 ...
	 * returning to the cache then insert it at the start so that it is likely
	 * to be returned by a subsequent call to getTemporaryDirectBuffer.
	 */
	...
	/**
	 * Releases a temporary buffer by returning to the cache or freeing it. If
	 * returning to the cache then insert it at the end. This makes it suitable
	 * for scatter/gather operations where the buffers are returned to cache in
	 * same order that they were obtained.
	 */
	...
	static Selector getTemporarySelector(SelectableChannel sc)
		...
		Selector sel = null;
		if (ref == null || ((selWrapper = ref.get()) == null)
	...
	static void load() {
		synchronized (Util.class) {
			...
			java.security.AccessController
					.doPrivileged(new sun.security.action.LoadLibraryAction(
							"net"));
			java.security.AccessController
					.doPrivileged(new sun.security.action.LoadLibraryAction(
							"nio"));
