...
final class WindowsSelectorImpl extends SelectorImpl {
	...
	private final static int MAX_SELECTABLE_FDS = 4;
	...
	private final class SubSelector {
		...
		private int poll() throws IOException { // poll for the main thread
			return poll0(pollWrapper.pollArrayAddress,
					Math.min(totalChannels, MAX_SELECTABLE_FDS), readFds,
		...
		private int poll(int index) throws IOException {
			...
			return poll0(
					pollWrapper.pollArrayAddress
							+ (pollArrayIndex * PollArrayWrapper.SIZE_POLLFD),
					Math.min(MAX_SELECTABLE_FDS, totalChannels - (index + 1)
							* MAX_SELECTABLE_FDS), readFds, writeFds,
					exceptFds, timeout);
		}
		...
		private int processSelectedKeys(long updateCount) {
			...
			numKeysUpdated += processFDSet(updateCount, readFds,
					PollArrayWrapper.POLLIN, false);
			numKeysUpdated += processFDSet(updateCount, writeFds,
					PollArrayWrapper.POLLCONN | PollArrayWrapper.POLLOUT, false);
			numKeysUpdated += processFDSet(updateCount, exceptFds,
					PollArrayWrapper.POLLIN | PollArrayWrapper.POLLCONN
							| PollArrayWrapper.POLLOUT, true);
			return numKeysUpdated;
		}

		/**
		 * Note, clearedCount is used to determine if the readyOps have been
		 * reset in this select operation. updateCount is used to tell if a key
		 * has been counted as updated in this select operation.
		 * 
		...
		private int processFDSet(long updateCount, int[] fds, int rOps,
			...
			for (int i = 1; i <= fds[0]; i++) {
				...
				if (isExceptFds && (sk.channel() instanceof SocketChannelImpl)
		...
		synchronized (closeLock) {
			if (channelArray != null) {
				if (pollWrapper != null) {
					...
					for (int i = 1; i < totalChannels; i++) { // Deregister
																// channels
